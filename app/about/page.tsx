...
export default function AboutPage() { return <><SiteHeader /><main><PageHero title="技術判断を、成果へつなぐ" lead={profile.summary} />
  <section className="about-roles-section" aria-labelledby="about-roles-title"><div className="container about-roles-layout"><div><p className="eyebrow">支援で発揮する能力</p><h2 id="about-roles-title">見立てから、<br />必要な技術検証まで</h2></div><ol>{profile.roles.map((role, index) => <li key={role.title}><span>{String(index + 1).padStart(2, "0")}</span><div><h3>{role.title}</h3><p>{role.text}</p></div></li>)}</ol></div></section>
