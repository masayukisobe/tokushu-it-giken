...
export default function Home() {
  return <><SiteHeader /><main>
    <section className="hero hero-decision" aria-labelledby="hero-title"><div className="hero-inner hero-layout">
      <div className="hero-message"><p className="eyebrow hero-eyebrow">TECHNICAL CONSULTATION</p><h1 id="hero-title">どこに頼めばいいか分からない。<br /><em>そんな技術課題に、<br />見立てと、次の一手を。</em></h1><p className="hero-copy">前例も、頼れる専門家も見つからない技術課題を、事業要件と現場の制約をふまえて解きほぐします。一次情報の調査と検証で判断材料を整え、必要に応じて設計・実装まで支援します。</p><div className="hero-actions"><Link className="button button-primary" href="/contact">30分無料で相談する <span aria-hidden="true">→</span></Link></div><div className="home-category-links" aria-label="相談できる代表的な課題"><p>相談できる代表的な課題</p>{consultationCategories.map((category) => <Link key={category.id} href={`/services#category-${category.id}`}><span>{category.id}</span><strong>{category.title}</strong><small>相談例を見る →</small></Link>)}</div></div>
    </div></section>
