...
export default function Home() {
  return <><SiteHeader /><main>
    <section className="hero hero-decision" aria-labelledby="hero-title"><div className="hero-grid" aria-hidden="true" /><div className="hero-inner hero-layout">
      <div className="hero-message"><h1 id="hero-title">どこに頼めばいいか分からない。<br /><em>そんな技術課題に、<br />見立てと、次の一手を。</em></h1><p className="hero-copy">相談先も進め方も定まらない技術課題を診断し、意思決定から実行設計、必要な技術検証まで支援します。前例が見つからない課題も、事業要件と現場の制約をふまえ、一次情報から読み解きます。</p><div className="hero-actions"><Link className="button button-primary" href="/contact">30分無料で相談する <span aria-hidden="true">→</span></Link></div></div>
    ...
    <section className="home-ai-section" aria-labelledby="home-ai-title"><div className="container home-ai-layout"><div><p className="eyebrow">AIに関する相談</p><h2 id="home-ai-title">AIも、導入ありきではなく、技術課題として見立てます。</h2><p>生成AI、RAG、AIエージェントなどについて、自社で使う意味、技術的な実現性、データ・精度・コスト・運用上の成立条件から判断します。AIを使わない選択も含め、調査・評価、実行設計、必要な技術検証まで支援します。</p></div><ul className="home-ai-links">{aiConsultationThemes.map((theme, index) => <li key={theme.id}><Link href={`/services/ai#${theme.id}`}><span>{String(index + 1).padStart(2, "0")}</span><strong>{theme.title}</strong><b aria-hidden="true">↗</b></Link></li>)}</ul></div></section>

    <section className="home-consultation-summary" aria-labelledby="consultation-summary-title"><div className="container home-consultation-layout"><div><p className="eyebrow">初回30分無料相談</p><h2 id="consultation-summary-title">まずは30分、判断を前へ進めます</h2><p>その場で回答できることは、その場で回答します。追加確認が必要な場合は、未確認事項、判断基準、担当すべき役割、確認する順序を明らかにします。30分で方向が定まれば、それで完了です。継続支援が必要な場合のみ、診断、評価、実行設計、必要な技術検証をご提案します。</p></div><Link className="button button-primary" href="/contact">30分無料で相談する <span aria-hidden="true">→</span></Link></div></section>
  </main><SiteFooter /></>;
}
